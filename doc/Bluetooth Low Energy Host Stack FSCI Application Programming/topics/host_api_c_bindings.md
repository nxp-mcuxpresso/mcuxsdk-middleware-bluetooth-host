...
The Host SDK includes a set of C bindings to interface with a _black-box_. Bindings are generated from the matching FSCI XML file that is available in the stack software package under `tools\wireless\xml_fsci`. The bindings are designed to be platform agnostic, with a minimal set of OS abstraction symbols required for building. Thus, the files can be easily integrated on a wide range of host platforms.
...
- Prototypes for sending commands
    ...
    ```
    that creates and sends a FSCI packet (0x02 | og | oc | msgLen | msg | crc +- fsciInterface) on the serial interface. Source files
    cmd_\<name\>.c, evt_\<name\>.c and evt_printer_\<name\>.c are generated from the correspondent \<NAME\>.xml FSCI XML file.
