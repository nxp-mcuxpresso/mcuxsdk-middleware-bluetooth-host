...
bleResult_t GattDbDynamic_AddGattService(gattServiceHandles_t* pOutServiceHandles)
{
    ...
    uint8_t        gattServiceChangedValue[] =
                        mGattDbDynamic_GattServiceChangedInitValue;

    serviceInfo_t* pServiceInfo              =
                        (serviceInfo_t*)MEM_BufferAlloc(
    ...
    if( NULL != pServiceInfo )
    {
        ...
        infoVars.serviceInfoTemp = ((uint8_t*)pServiceInfo +
                                    sizeof(serviceInfo_t));
        ...
        pServiceInfo->pCharacteristicInfo[0].properties =
                    (gattCharacteristicPropertiesBitFields_t)((uint8_t)gGattCharPropRead_c |
                                                              (uint8_t)gGattCharPropNotify_c);
        pServiceInfo->pCharacteristicInfo[0].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[0].valueLength =
                    (uint16_t)sizeof(gattServiceChangedValue);
        pServiceInfo->pCharacteristicInfo[0].pValue =
                    (uint8_t*)pServiceInfo->pCharacteristicInfo +
                    sizeof(characteristicInfo_t);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].pValue,
                    &gattServiceChangedValue,
                    sizeof(gattServiceChangedValue));
        pServiceInfo->pCharacteristicInfo[0].accessPermissions =
                    gPermissionNone_c;
        ...
        if( (gBleSuccess_c == result) && (NULL != pOutServiceHandles) )
        {
            ...
            pOutServiceHandles->charServiceChangedHandle =
                    pServiceInfo->pCharacteristicInfo->handle;
            pOutServiceHandles->charServiceChangedCccdHandle =
                    pServiceInfo->pCharacteristicInfo->cccdHandle;
        }
...
bleResult_t GattDbDynamic_AddGapService(gapServiceHandles_t* pOutServiceHandles)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        infoVars.serviceInfoTemp = ((uint8_t*)pServiceInfo +
                                    sizeof(serviceInfo_t));
        pServiceInfo->pCharacteristicInfo =
                    infoVars.characteristicInfoTemp;
        pServiceInfo->pCharacteristicInfo[0].uuid16 =
                    gBleSig_GapDeviceName_d;
        pServiceInfo->pCharacteristicInfo[0].properties =
                    (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[0].maxValueLength = 20;
        pServiceInfo->pCharacteristicInfo[0].valueLength =
                    (uint16_t)sizeof(gapDeviceNameValue);
        pServiceInfo->pCharacteristicInfo[0].pValue =
                    (uint8_t*)pServiceInfo->pCharacteristicInfo +
                    3U * sizeof(characteristicInfo_t);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].pValue,
        ...
        pServiceInfo->pCharacteristicInfo[1].properties =
                    (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[1].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[1].valueLength =
                    (uint16_t)sizeof(gapAppearanceValue);
        pServiceInfo->pCharacteristicInfo[1].pValue =
                    pServiceInfo->pCharacteristicInfo[0].pValue +
                    sizeof(gapDeviceNameValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[1].pValue,
                    &gapAppearanceValue,
                    sizeof(gapAppearanceValue));
        pServiceInfo->pCharacteristicInfo[1].accessPermissions =
                    gPermissionFlagReadable_c;
        ...
        pServiceInfo->pCharacteristicInfo[2].properties =
                    (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[2].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[2].valueLength =
                    (uint16_t)sizeof(gapPpcpValue);
        pServiceInfo->pCharacteristicInfo[2].pValue =
                    pServiceInfo->pCharacteristicInfo[1].pValue +
                    sizeof(gapAppearanceValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[2].pValue,
                    &gapPpcpValue,
                    sizeof(gapPpcpValue));
        pServiceInfo->pCharacteristicInfo[2].accessPermissions =
                    gPermissionFlagReadable_c;
        ...
        if( (gBleSuccess_c == result) && (NULL != pOutServiceHandles) )
        {
            /* Fill service handles */
            pOutServiceHandles->serviceHandle = pServiceInfo->handle;
            pOutServiceHandles->charDeviceNameHandle =
                    pServiceInfo->pCharacteristicInfo[0].handle;
            pOutServiceHandles->charAppearanceHandle =
                    pServiceInfo->pCharacteristicInfo[1].handle;
            pOutServiceHandles->charPpcpHandle =
                    pServiceInfo->pCharacteristicInfo[2].handle;
        }
...
)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        if( (gBleSuccess_c == result) && (NULL != pOutServiceHandles) )
        {
            /* Fill service handles */
            pOutServiceHandles->serviceHandle = pServiceInfo->handle;
            pOutServiceHandles->charHrMeasurementHandle =
                        pServiceInfo->pCharacteristicInfo[0].handle;
            pOutServiceHandles->charHrMeasurementCccdHandle =
                        pServiceInfo->pCharacteristicInfo[0].cccdHandle;
            pOutServiceHandles->charBodySensorLocationHandle =
                        pServiceInfo->pCharacteristicInfo[1].handle;
            pOutServiceHandles->charHrCtrlPointHandle =
                        pServiceInfo->pCharacteristicInfo[2].handle;
        }
...
)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        if( (gBleSuccess_c == result) && (NULL != pOutServiceHandles) )
        {
            /* Fill service handles */
            pOutServiceHandles->serviceHandle = pServiceInfo->handle;
            pOutServiceHandles->charBatteryLevelHandle =
                    pServiceInfo->pCharacteristicInfo[0].handle;
            pOutServiceHandles->charBatteryLevelCccdHandle =
                    pServiceInfo->pCharacteristicInfo[0].cccdHandle;
        }
...
)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        if( (gBleSuccess_c == result) && (NULL != pOutServiceHandles) )
        {
            /* Fill service handles */
            pOutServiceHandles->serviceHandle = pServiceInfo->handle;
            pOutServiceHandles->charManufNameHandle =
                    pServiceInfo->pCharacteristicInfo[0].handle;
            pOutServiceHandles->charModelNoHandle =
                    pServiceInfo->pCharacteristicInfo[1].handle;
            pOutServiceHandles->charSerialNoHandle =
                    pServiceInfo->pCharacteristicInfo[2].handle;
            pOutServiceHandles->charHwRevHandle =
                    pServiceInfo->pCharacteristicInfo[3].handle;
            pOutServiceHandles->charFwRevHandle =
                    pServiceInfo->pCharacteristicInfo[4].handle;
            pOutServiceHandles->charSwRevHandle =
                    pServiceInfo->pCharacteristicInfo[5].handle;
            pOutServiceHandles->charSystemIdHandle =
                    pServiceInfo->pCharacteristicInfo[6].handle;
            pOutServiceHandles->charRcdlHandle =
                    pServiceInfo->pCharacteristicInfo[7].handle;
        }
...
*\brief         Populates a service info structure with Heart Rate service
*               information.
*
*\param  [in]   none.
*
...
serviceInfo_t* GattDbDynamic_GetHeartRateService(void)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        pServiceInfo->pCharacteristicInfo[0].valueLength =
                        (uint16_t)sizeof(hrHrMeasurementValue);
        pServiceInfo->pCharacteristicInfo[0].pValue =
                        (uint8_t*)pServiceInfo->pCharacteristicInfo +
                        3U * sizeof(characteristicInfo_t);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].pValue,
                        &hrHrMeasurementValue,
                        sizeof(hrHrMeasurementValue));
        pServiceInfo->pCharacteristicInfo[0].accessPermissions =
                        gPermissionNone_c;
        pServiceInfo->pCharacteristicInfo[0].bAddCccd = TRUE;
        ...
        pServiceInfo->pCharacteristicInfo[1].uuid16 =
                        gBleSig_BodySensorLocation_d;
        pServiceInfo->pCharacteristicInfo[1].properties =
                        (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[1].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[1].valueLength =
                        (uint16_t)sizeof(hrBodySensorLocationValue);
        pServiceInfo->pCharacteristicInfo[1].pValue =
                        pServiceInfo->pCharacteristicInfo[0].pValue +
                        sizeof(hrHrMeasurementValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[1].pValue,
                        &hrBodySensorLocationValue,
                        sizeof(hrBodySensorLocationValue));
        pServiceInfo->pCharacteristicInfo[1].accessPermissions =
                        gPermissionFlagReadable_c;
        pServiceInfo->pCharacteristicInfo[1].bAddCccd = FALSE;
        ...
        pServiceInfo->pCharacteristicInfo[2].uuid16 =
                        gBleSig_HrControlPoint_d;
        pServiceInfo->pCharacteristicInfo[2].properties =
                        (uint8_t)gGattCharPropWrite_c;
        pServiceInfo->pCharacteristicInfo[2].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[2].valueLength =
                    (uint16_t)sizeof(hrHrControlPointValue);
        pServiceInfo->pCharacteristicInfo[2].pValue =
                    pServiceInfo->pCharacteristicInfo[1].pValue +
                    sizeof(hrBodySensorLocationValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[2].pValue,
                    &hrHrControlPointValue,
                    sizeof(hrHrControlPointValue));
        pServiceInfo->pCharacteristicInfo[2].accessPermissions =
...
serviceInfo_t* GattDbDynamic_GetBatteryService(void)
{
    ...
    uint8_t bBatteryLevelValue[] =
                mGattDbDynamic_BServiceLevelInitValue;

    uint8_t bCharPresFormatDescriptorValue[] =
    ...
    if( NULL != pServiceInfo )
    {
        ...
        pServiceInfo->pCharacteristicInfo[0].properties =
                (gattCharacteristicPropertiesBitFields_t)((uint8_t)gGattCharPropNotify_c |
                                                          (uint8_t)gGattCharPropRead_c);
        pServiceInfo->pCharacteristicInfo[0].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[0].valueLength =
                (uint16_t)sizeof(bBatteryLevelValue);
        pServiceInfo->pCharacteristicInfo[0].pValue =
                (uint8_t*)pServiceInfo->pCharacteristicInfo +
                1U * sizeof(characteristicInfo_t) +
                1U * sizeof(descriptorInfo_t);
        ...
        infoVars.serviceInfoTemp =
                    ((uint8_t*)pServiceInfo->pCharacteristicInfo +
                    1U * sizeof(characteristicInfo_t));
        pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo =
                    infoVars.descriptorInfoTemp;

        pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo[0].uuid16 =
                    gBleSig_CharPresFormatDescriptor_d;
        pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo[0].valueLength =
                    (uint16_t)sizeof(bCharPresFormatDescriptorValue);
        pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo[0].pValue =
                    (uint8_t*)pServiceInfo->pCharacteristicInfo[0].pValue +
                    sizeof(bBatteryLevelValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo[0].pValue,
                    &bCharPresFormatDescriptorValue,
                    sizeof(bCharPresFormatDescriptorValue));
        pServiceInfo->pCharacteristicInfo[0].pDescriptorInfo[0].accessPermissions =
                    gPermissionFlagReadable_c;
    }
...
serviceInfo_t* GattDbDynamic_GetDeviceInformationService(void)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        pServiceInfo->pCharacteristicInfo[0].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[0].valueLength =
                (uint16_t)sizeof(diManufacturerNameValue);
        pServiceInfo->pCharacteristicInfo[0].pValue =
                (uint8_t*)pServiceInfo->pCharacteristicInfo +
                8U * sizeof(characteristicInfo_t);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].pValue,
                    &diManufacturerNameValue,
                    sizeof(diManufacturerNameValue));
        pServiceInfo->pCharacteristicInfo[0].accessPermissions =
                    gPermissionFlagReadable_c;
        pServiceInfo->pCharacteristicInfo[0].bAddCccd = FALSE;
        ...
        pServiceInfo->pCharacteristicInfo[1].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[1].valueLength =
                    (uint16_t)sizeof(diModelNumberValue);
        pServiceInfo->pCharacteristicInfo[1].pValue =
                    pServiceInfo->pCharacteristicInfo[0].pValue +
                    sizeof(diManufacturerNameValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[1].pValue,
        ...
        pServiceInfo->pCharacteristicInfo[2].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[2].valueLength =
                    (uint16_t)sizeof(diSerialNumberValue);
        pServiceInfo->pCharacteristicInfo[2].pValue =
                    pServiceInfo->pCharacteristicInfo[1].pValue +
                    sizeof(diModelNumberValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[2].pValue,
        ...
        pServiceInfo->pCharacteristicInfo[3].uuid16 =
                gBleSig_HardwareRevisionString_d;
        pServiceInfo->pCharacteristicInfo[3].properties =
                (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[3].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[3].valueLength =
                    (uint16_t)sizeof(diHardwareRevisionValue);
        pServiceInfo->pCharacteristicInfo[3].pValue =
                    pServiceInfo->pCharacteristicInfo[2].pValue +
                    sizeof(diSerialNumberValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[3].pValue,
                    &diHardwareRevisionValue,
                    sizeof(diHardwareRevisionValue));
        pServiceInfo->pCharacteristicInfo[3].accessPermissions =
                gPermissionFlagReadable_c;
        ...
        pServiceInfo->pCharacteristicInfo[4].uuid16 =
                gBleSig_FirmwareRevisionString_d;
        pServiceInfo->pCharacteristicInfo[4].properties =
                (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[4].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[4].valueLength =
                (uint16_t)sizeof(diFirmwareRevisionValue);
        pServiceInfo->pCharacteristicInfo[4].pValue =
                pServiceInfo->pCharacteristicInfo[3].pValue +
                sizeof(diHardwareRevisionValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[4].pValue,
                    &diFirmwareRevisionValue,
                    sizeof(diFirmwareRevisionValue));
        pServiceInfo->pCharacteristicInfo[4].accessPermissions =
                gPermissionFlagReadable_c;
        ...
        pServiceInfo->pCharacteristicInfo[5].uuid16 =
                gBleSig_SoftwareRevisionString_d;
        pServiceInfo->pCharacteristicInfo[5].properties =
                (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[5].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[5].valueLength =
                (uint16_t)sizeof(diSoftwareRevisionValue);
        pServiceInfo->pCharacteristicInfo[5].pValue =
                pServiceInfo->pCharacteristicInfo[4].pValue +
                sizeof(diFirmwareRevisionValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[5].pValue,
                    &diSoftwareRevisionValue,
                    sizeof(diSoftwareRevisionValue));
        pServiceInfo->pCharacteristicInfo[5].accessPermissions =
                gPermissionFlagReadable_c;
        ...
        pServiceInfo->pCharacteristicInfo[6].uuid16 =
                gBleSig_SystemId_d;
        pServiceInfo->pCharacteristicInfo[6].properties =
                (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[6].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[6].valueLength =
                (uint16_t)sizeof(diSystemIdValue);
        pServiceInfo->pCharacteristicInfo[6].pValue  =
                pServiceInfo->pCharacteristicInfo[5].pValue +
                sizeof(diSoftwareRevisionValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[6].pValue,
                    &diSystemIdValue,
                    sizeof(diSystemIdValue));
        pServiceInfo->pCharacteristicInfo[6].accessPermissions =
                gPermissionFlagReadable_c;
        ...
        pServiceInfo->pCharacteristicInfo[7].uuid16 =
                gBleSig_IeeeRcdl_d;
        pServiceInfo->pCharacteristicInfo[7].properties =
                (uint8_t)gGattCharPropRead_c;
        pServiceInfo->pCharacteristicInfo[7].maxValueLength = 0;
        pServiceInfo->pCharacteristicInfo[7].valueLength =
                (uint16_t)sizeof(diIeeeRcdlValue);
        pServiceInfo->pCharacteristicInfo[7].pValue =
                pServiceInfo->pCharacteristicInfo[6].pValue +
                sizeof(diSystemIdValue);
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[7].pValue,
                    &diIeeeRcdlValue,
                    sizeof(diIeeeRcdlValue));
        pServiceInfo->pCharacteristicInfo[7].accessPermissions =
                gPermissionFlagReadable_c;
