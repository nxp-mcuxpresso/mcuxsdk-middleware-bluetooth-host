...
#if defined(gRasRapPtsTest_d) && (gRasRapPtsTest_d == 1)
        /* Share local data and result handlers with the PTS test engine */
        PtsTest_RegisterCallbacks(processCsResultsEvent, processCsResultsContinueEvent);
#endif /* defined(gRasRapPtsTest_d) && (gRasRapPtsTest_d == 1) */
...
)
{
    ...
    {
        *pOutMaxProcDuration = UINT16_MAX;
    }
    else
    {
        *pOutMaxProcDuration = (uint16_t)maxProcDuration;
    }
}
/*! *********************************************************************************
