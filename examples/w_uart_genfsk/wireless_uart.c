...
static bool_t BleApp_CheckExistingServiceHandles(deviceId_t peerDeviceId);
...
static void BleApp_StateMachineHandleIdle(deviceId_t peerDeviceId, appEvent_t event, bleUuid_t *pUuidObj);
    ...
    if (maPeerInformation[peerDeviceId].deviceId != gInvalidDeviceId_c)
    {
        ...
        case mAppExchangeMtu_c:
        {
            if (event == mAppEvt_GattProcComplete_c)
            {
                ...
                if (BleApp_CheckExistingServiceHandles(peerDeviceId))
                {
                    maPeerInformation[peerDeviceId].appState = mAppRunning_c;
                }
                else
...
static void GenfskApp_EventHandler(const void *pGfskAppData)
{
    ...
    {
        const gfskAppEventData_t *pAppData = (const gfskAppEventData_t*)pGfskAppData;

        switch (pAppData->appEvent)
        {
            case gGfskEvt_MetaEventTransmitComplete_c:
            {
                Serial_Print("\n\rGFSK: Transmit Complete", gAllowToBlock_d);
                /* After any Genfsk print, use newline for uart */
                mAppUartNewLine = TRUE;
            }
            break;
