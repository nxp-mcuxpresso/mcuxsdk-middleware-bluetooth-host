...
bleResult_t Hts_SetMeasurementInterval(htsConfig_t *pServiceConfig, uint16_t measurementInterval)
{
    ...
    if (result == gBleSuccess_c)
    {
        ...
        result = GattDb_WriteAttribute(handle, (uint16_t)sizeof(uint16_t), (uint8_t*)&measurementInterval);
...
attErrorCode_t Hts_MeasurementIntervalWriting(htsConfig_t *pServiceConfig, uint16_t handle, uint16_t newMeasurementInterval)
{
    ...
    uint16_t  descHandle = 0U, minValue = 0U, maxValue = 0U, descValueLength = 0U;
    uint8_t aRangeValues[4] = {0U};
...
)
{
    ...
    return GattDb_WriteAttribute(handle, (uint16_t)index, &charValue[0]);
}
...
)
{
    ...
    if(result == gBleSuccess_c)
    {
        result = GattDb_WriteAttribute(hValue, (uint16_t)sizeof(htsTempType_t), &tempType);
    }
