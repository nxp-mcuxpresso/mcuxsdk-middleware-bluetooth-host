    ...
    else
    {
        ...
        if(TRUE == bFsciBleGapHandoverEnabled)
        {
            ...
            switch(pClientPacket->structured.header.opCode)
            {
                ...
                case gBleGapHandoverSetLlPendingDataOpCode_c:
                    {
                        uint16_t connectionHandle = 0U;
                        uint16_t dataSize = 0U;
                        
                        fsciBleGetUint16ValueFromBuffer(connectionHandle, pBuffer);
                        fsciBleGetUint16ValueFromBuffer(dataSize, pBuffer);
                        (void)dataSize;
