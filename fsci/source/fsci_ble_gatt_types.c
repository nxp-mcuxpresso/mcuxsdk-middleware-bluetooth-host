...
#define getIndexFromDeviceIdBearerId(deviceId, bearerId) \
        ((deviceId) * (gBleEattMaxConnectionChannels + 1U) + (bearerId))
...
static fsciBleGattClientManagementInfo_t            fsciBleGattClientAllocatedManagementInfo[gAppMaxConnections_c * (1U + gAppEattMaxNoOfBearers_c)]    = {0};  /* Keeps information about the allocated
                                                                                                           management info */
static fsciBleGattClientManagementInfo_t            fsciBleGattClientTmpAllocatedManagementInfo[gAppMaxConnections_c * (1U + gAppEattMaxNoOfBearers_c)] = {0};  /* Keeps information about the temporary
                                                                                                           allocated management info */

static fsciBleGattClientManagementInfo_t            fsciBleGattClientSavedManagementInfo[gFsciBleGattMaximumActiveConnections_c * (1U + gAppEattMaxNoOfBearers_c)];
...
static bool_t checkAllocatedClientManagementInfo(uint32_t idx);
/************************************************************************************
*************************************************************************************
* Public functions
*************************************************************************************
...
)
{
    ...
    return fsciBleGattClientTmpAllocatedManagementInfo[idx].pValue;
}
...
uint32_t fsciBleGattClientGetCharacteristicBufferSize(const gattCharacteristic_t* pCharacteristic)
{
    /* Get the constant size for the buffer needed by a characteristic */
    uint32_t bufferSize = sizeof(gattCharacteristicPropertiesBitFields_t) +
...
static uint32_t fsciBleGattClientGetServiceBufferSizeIterative(const gattService_t* pService)
{
    ...
    uint32_t bufferSize = sizeof(uint16_t) + sizeof(uint16_t) + sizeof(bleUuidType_t) +
...
uint32_t fsciBleGattClientGetServiceBufferSize(const gattService_t* pService)
{
    uint32_t iCount     = 0U;
...
void fsciBleGattClientSaveServicesInfo(deviceId_t deviceId, bearerId_t bearerId, gattService_t* pServices)
{
    /* Keep (only save) the services array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    fsciBleGattClientSavedManagementInfo[idx].pServices = pServices;
}


void fsciBleGattClientSaveIncludedServicesInfo(deviceId_t deviceId, bearerId_t bearerId, gattService_t* pIncludedServices)
{
    /* Keep (only save) the includedServices array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    fsciBleGattClientSavedManagementInfo[idx].pIncludedServices = pIncludedServices;
}


void fsciBleGattClientSaveCharacteristicsInfo(deviceId_t deviceId, bearerId_t bearerId, gattCharacteristic_t* pCharacteristics)
{
    /* Keep (only save) the characteristics array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    fsciBleGattClientSavedManagementInfo[idx].pCharacteristics = pCharacteristics;
}


void fsciBleGattClientSaveDescriptorsInfo(deviceId_t deviceId, bearerId_t bearerId, gattAttribute_t* pDescriptors)
{
    /* Keep (only save) the descriptors array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    fsciBleGattClientSavedManagementInfo[idx].pDescriptors = pDescriptors;
}


void fsciBleGattClientSaveValueInfo(deviceId_t deviceId, bearerId_t bearerId, uint8_t* pValue)
{
    /* Keep (only save) the value */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    fsciBleGattClientSavedManagementInfo[idx].pValue = pValue;
}


void fsciBleGattClientSaveArraySizeInfo(deviceId_t deviceId, bearerId_t bearerId, uint16_t* pArraySize)
{
...
gattService_t* fsciBleGattClientGetServicesInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
    /* Return the kept (allocated or saved) services array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    return (TRUE == bAllocated) ? fsciBleGattClientAllocatedManagementInfo[idx].pServices : fsciBleGattClientSavedManagementInfo[idx].pServices;
}


gattService_t* fsciBleGattClientGetIncludedServicesInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
    /* Return the kept (allocated or saved) includedServices array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    return (TRUE == bAllocated) ? fsciBleGattClientAllocatedManagementInfo[idx].pIncludedServices : fsciBleGattClientSavedManagementInfo[idx].pIncludedServices;
}


gattCharacteristic_t* fsciBleGattClientGetCharInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
    /* Return the kept (allocated or saved) characteristics array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    return (TRUE == bAllocated) ? fsciBleGattClientAllocatedManagementInfo[idx].pCharacteristics : fsciBleGattClientSavedManagementInfo[idx].pCharacteristics;
}


gattAttribute_t* fsciBleGattClientGetDescriptorsInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
    /* Return the kept (allocated or saved) descriptors array */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    return (TRUE == bAllocated) ? fsciBleGattClientAllocatedManagementInfo[idx].pDescriptors : fsciBleGattClientSavedManagementInfo[idx].pDescriptors;
}


uint8_t* fsciBleGattClientGetValueInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
    /* Return the kept (allocated or saved) value */
    uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
    return (TRUE == bAllocated) ? fsciBleGattClientAllocatedManagementInfo[idx].pValue : fsciBleGattClientSavedManagementInfo[idx].pValue;
}


uint16_t* fsciBleGattClientGetArraySizeInfo(deviceId_t deviceId, bearerId_t bearerId, bool_t bAllocated)
{
...
{
    for (uint32_t idx = 0U; idx < gAppMaxConnections_c * (1U + gAppEattMaxNoOfBearers_c); idx++)
    {
        /* Keep temporary information */
        if ((TRUE == bAllocatedInfo) && (checkAllocatedClientManagementInfo(idx) == FALSE))
        {
            /* Keep temporary allocated information */
            FLib_MemCpy(&fsciBleGattClientAllocatedManagementInfo[idx], &fsciBleGattClientTmpAllocatedManagementInfo[idx], sizeof(fsciBleGattClientManagementInfo_t));
            /* Erase the temporary allocated information */
            FLib_MemSet(&fsciBleGattClientTmpAllocatedManagementInfo[idx], 0x00, sizeof(fsciBleGattClientManagementInfo_t));
        }
    }
}
...
)
{
    ...
    if ((TRUE == bAllocatedInfo) && (gInvalidDeviceId_c != deviceId))
    {
        /* Erase allocated (temporary or kept) information */
        uint8_t idx = getIndexFromDeviceIdBearerId(deviceId, bearerId);
        fsciBleGattClientManagementInfo_t* pFsciBleGattClientAllocatedManagementInfo = (TRUE == bTmp) ? &fsciBleGattClientTmpAllocatedManagementInfo[idx] : &fsciBleGattClientAllocatedManagementInfo[idx];
    ...
    else
    {
        ...
        FLib_MemSet(&fsciBleGattClientSavedManagementInfo, 0x00, sizeof(fsciBleGattClientManagementInfo_t) * gAppMaxConnections_c * (1U + gAppEattMaxNoOfBearers_c));
    }
}
...
uint32_t fsciBleGattServerGetServerEventBufferSize(gattServerEvent_t* pServerEvent)
{
...
static void fsciBleGattClientHandleErrorStatus(gFsciStatus_t status)
{
    ...
    fsciBleGattClientEraseTmpInfo(TRUE, gInvalidDeviceId_c, gUnenhancedBearerId_c);
}

static bool_t checkAllocatedClientManagementInfo
(
    uint32_t idx
)
{
    ...
    if ((fsciBleGattClientAllocatedManagementInfo[idx].pServices != NULL)
