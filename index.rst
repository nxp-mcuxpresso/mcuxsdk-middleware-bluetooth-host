.. _examples__wireless_examples__bluetooth_docs:
NXP Bluetooth LE Host and Sample Applications
==============================================

.. toctree::
  :maxdepth: 1
  
  :caption Changelog
  
  CHANGELOG <CHANGELOG.md>  
  
  :caption NXP Bluetooth LE Host Documentation
  Bluetooth Low Energy Application Developer’s Guide <doc/Bluetooth Low Energy Application Developers Guide/Bluetooth Low Energy Application Developers Guide.md>
  Bluetooth Low Energy Software Quick Start Guide <doc/Bluetooth Low Energy Quick Start Guide/Bluetooth Low Energy Quick Start Guide.md>
  Bluetooth Low Energy Demo Applications User Guide <doc/Bluetooth Low Energy Demo Applications Users Guide/Bluetooth Low Energy Demo Applications Users Guide.md>
  Bluetooth Low Energy CCC Digital Key R3 Application Note <doc/Bluetooth Low Energy CCC Digital Key Application Note/Bluetooth Low Energy CCC Digital Key Application Note.md>
  Bluetooth Low Energy Host Stack FSCI Application Programming <doc/Bluetooth Low Energy Host Stack FSCI Application Programming/Bluetooth Low Energy Host Stack FSCI Application Programming.md>
  Bluetooth Low Energy Connection Handover <doc/Bluetooth Low Energy Connection Handover/Bluetooth Low Energy Connection Handover.md>
  doc/Bluetooth Low Energy Host Stack API Reference Manual/doxy.rst

  Bluetooth Low Energy Localization Application Note <doc/Bluetooth Low Energy Localization Application Note/Bluetooth Low Energy Localization Application Note.md>
  Bluetooth Low Energy Channel Sounding Application Developers Guide <doc/Bluetooth Low Energy Channel Sounding Application Developers Guide/Bluetooth Low Energy Channel Sounding Application Developers Guide.md>
  Bluetooth Channel Sounding Technique for Distance Estimation <doc/Bluetooth Channel Sounding Technique for Distance Estimation/Bluetooth Channel Sounding Technique for Distance Estimation.md>
  Bluetooth Low Energy CCC Digital Key with Channel Sounding Application Note <doc/Bluetooth Low Energy CCC Digital Key with Channel Sounding Application Note/Bluetooth Low Energy CCC Digital Key with Channel Sounding Application Note.md>
  CS Wireless Ranging Demo Application User Guide <doc/CS Wireless Ranging Demo Application User Guide/CS Wireless Ranging Demo Application User Guide.md>